    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('건강 정보 조회 실패: ${e.message}')),
        );
      }
    } finally {
      if (mounted) {
  ...
  Future<void> _saveHealthProfile() async {
    ...
    try {
      await _profileApi.saveHealthProfile(
        disease: _selectedDiseases.isEmpty ? '없음' : _selectedDiseases.join(', '),
          ...
          children: [
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: Text(_isSaving ? '저장 중...' : '건강 정보 저장하기', style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)),
              ),
            ),
